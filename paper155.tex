...
\label{sec:results}
%
We use cardiac ultrasound videos and associated ECG recordings of 6 mice to validate the proposed methods. The ultrasound videos were acquired using the VisualSonics Vevo 2100 scanner at 233 frames per second. Furthermore, each video consisted of approximately 300 frames, 11 cardiac cycles, and 2 respiratory cycles.

	We validate our cardiac phase estimation method by comparing its phase estimates with the ECG signal. Figure~\ref{fig:instaphase_vs_ecg}(a) shows a head-to-head comparison between the ECG signal and the instantaneous cardiac phase obtained using the method described in Section~\ref{sec:method:phase_estimation} for one of the 6 videos. 
...
\hline
\begin{tabular}[c]{@{}c@{}}VID\\ $\;$ \end{tabular} & \begin{tabular}[c]{@{}c@{}}$R^2$\\ $\;$ \end{tabular} & \begin{tabular}[c]{@{}c@{}}QRS Peak Prediction Error\\ Mean $\pm$ Std\end{tabular} \\ \hline
...
\end{tabular}
\caption{Comparision between frame positions of QRS peaks in the ECG signal and the corresponding minima of the cardiac phase estimated using our method for 6 videos}
